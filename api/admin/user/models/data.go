...
type (
	User struct {
		...
		Birthday       int64         `json:"birthday" gorm:"type:bigint;default=0;comment:出生日期"`
		...
		MobilePhone    string        `json:"mobile_phone" gorm:"size:14;comment:手机号码"`
		IdentityTag    int8          `json:"identity_tag" gorm:"not null;default=0;comment:身份标签，取值为[0:未选择, 1:母胎单身, 2:未婚单身, 3:离异无孩, 4:离异带孩, 5:离异不带孩, 6:恋爱中]"`
		IsVip          int8          `json:"is_vip" gorm:"not null;default:0;comment:是否vip，取值为[0:未知, 1:是, 2:否]"`
		VipTag         int32         `json:"vip_tag" gorm:"type:int;not null;default:0;comment:vip标签Id"`
		...
		CreatedAt      int64         `json:"created_at" gorm:"type:bigint;not null;comment:注册时间"`
		UpdatedAt      int64         `json:"updated_at" gorm:"type:bigint;not null;comment:修改时间"`
		DeletedAt      sql.NullInt64 `json:"deleted_at" gorm:"type:bigint;comment:注销时间"`
		IsDel          int8          `json:"is_del" gorm:"default:0;comment:删除标志，取值为[0:使用中, 1:已注销]"`
	}
	...
	MatchSetting struct {
		Id              int64  `json:"id" gorm:"type:bigint;primaryKey;autoIncrement;not null;comment:主键"`
		UserId          int64  `json:"user_id" gorm:"not null;comment:用户唯一id;index:idx_user_id"`
		TargetDistance  string `json:"ta_distance" gorm:"size:12;comment:希望与另一半的距离，单位km，中间使用英文横杠隔开，示例：1-20"`
		TargetAge       string `json:"ta_age" gorm:"size:12;comment:希望另一半年龄范围，单位岁，中间使用英文横杠隔开，示例：18-38"`
		TargetHeight    string `json:"ta_height" gorm:"size:12;comment:希望另一半身高范围，单位cm，中间使用英文横杠隔开，示例170-190"`
	...
	UserManagement struct {
		...
		Until     sql.NullInt64 `json:"until" gorm:"type:bigint;comment:解封时间"`
	...
	Reports struct {
		...
		ReportTime     int64  `json:"report_time" gorm:"type:bigint;not null;comment:举报时间"`
		UpdatedAt      int64  `json:"updated_at" gorm:"type:bigint;not null;comment:修改时间"`
	}
	...
	FriendRequest struct {
		...
		ReceiverReadTime    sql.NullInt64 `json:"receiver_read_time" gorm:"type:bigint;comment:被申请人首次查看到申请的时间"`
		ReceiverConfirmTime sql.NullInt64 `json:"receiver_confirm_time" gorm:"type:bigint;comment:被申请人回复申请时间"`
		CreatedAt           int64         `json:"created_at" gorm:"type:bigint;not null;comment:创建时间"`
		UpdatedAt           int64         `json:"updated_at" gorm:"type:bigint;not null;comment:修改时间"`
	}
	...
	FindCompanionActivity struct {
		...
		ActivityTime     int64         `json:"activity_time" gorm:"type:bigint;not null;comment:活动时间"`
		...
		CancelTime       sql.NullInt64 `json:"cancel_time" gorm:"type:bigint;comment:取消活动的时间"`
		CreatedAt        int64         `json:"created_at" gorm:"type:bigint;not null;comment:创建时间"`
		UpdatedAt        int64         `json:"updated_at" gorm:"type:bigint;not null;comment:修改时间"`
	}
	...
	FindCompanionRequest struct {
		...
		ApplicationTime int64         `json:"application_time" gorm:"type:bigint;not null;comment:申请参与活动的时间"`
		Status          int8          `json:"status" gorm:"not null;comment:申请状态，取值为[1:申请待处理中, 2:已通过申请, 3:已取消申请]"`
		CancelTime      sql.NullInt64 `json:"cancel_time" gorm:"type:bigint;comment:申请人取消参与活动的时间"`
		ApprovedTime    sql.NullInt64 `json:"approved_time" gorm:"type:bigint;comment:申请被活动发起人同意的时间"`
		CreatedAt       int64         `json:"created_at" gorm:"type:bigint;not null;comment:创建时间"`
		UpdatedAt       int64         `json:"updated_at" gorm:"type:bigint;not null;comment:修改时间"`
	}

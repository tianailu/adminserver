...
func TestJWTRace(t *testing.T) {
	...
	h := JWTWithConfig(JWTConfig{
		...
		Signing: map[string]interface{}{AlgorithmHS256: validKey},
	})(handler)
...
func TestJWT(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			config: JWTConfig{
				Signing: map[string]interface{}{"RS256": validKey},
			},
		...
		},
		{
			...
			hdrAuth:    validAuth,
			config: JWTConfig{
				Signing: map[string]interface{}{"HS256": invalidKey},
			},
			info: "Invalid key",
		},
		{
			hdrAuth: validAuth,
			config: JWTConfig{
				Signing: map[string]interface{}{"HS256": validKey},
			},
			info: "Valid JWT",
		},
		{
		...
		},
		{
			...
			config: JWTConfig{
				Claims:  &jwtCustomClaims{},
				Signing: map[string]interface{}{"HS256": []byte("secret")},
			},
